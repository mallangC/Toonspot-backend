...
export async function getKakaoPDailyToons(): Promise<SyncResult> {
  console.log("[데일리] 카카오 페이지 데이터 수집 시작");
  ...
  console.log(`[데일리] 카카오 페이지 수집 완료 - 갯수 ${result.createData.length + result.updateData.length}개, 소요 시간: ${duration}초`);
  return result;
}